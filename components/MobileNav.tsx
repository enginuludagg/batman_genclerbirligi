...
const MobileNav: React.FC<MobileNavProps> = ({ activeView, onViewChange, onToggleSidebar, appMode }) => {
  return (
    <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-6 py-3 pb-[calc(env(safe-area-inset-bottom)+16px)] z-[9999] flex justify-between items-center shadow-[0_-10px_40px_rgba(0,0,0,0.2)]">
      
      <button 
        onClick={() => onViewChange('dashboard')}
        className={`flex flex-col items-center gap-1 transition-all active:scale-90 ${activeView === 'dashboard' ? 'text-[#E30613]' : 'text-gray-400'}`}
      >
      ...
      <button 
        ...
        className={`flex flex-col items-center gap-1 transition-all active:scale-90 ${activeView === 'schedule' ? 'text-[#E30613]' : 'text-gray-400'}`}
      >
      ...
      <button 
        ...
        className={`flex flex-col items-center gap-1 transition-all active:scale-90 ${['finance', 'league'].includes(activeView) ? 'text-[#E30613]' : 'text-gray-400'}`}
      >
        {appMode === 'admin' ? (
      ...
      <button 
        ...
        className={`flex flex-col items-center gap-1 transition-all active:scale-90 ${activeView === 'students' ? 'text-[#E30613]' : 'text-gray-400'}`}
      >
      ...
      <button 
        ...
        className="flex flex-col items-center gap-1 text-gray-400 active:text-black transition-colors active:scale-90"
      >
        <Menu size={24} />
      </button>

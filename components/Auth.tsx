...
import { Mail, Lock, UserPlus, ArrowRight, AlertCircle, Sparkles, CheckCircle2 } from 'lucide-react';
...
const Auth: React.FC<AuthProps> = ({ onLogin, onRegisterStudent }) => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    const admins = ['enginuludagg@gmail.com', 'elitgelisimakademi@gmail.com'];
    const adminPassword = 'Eu290202';

    if (admins.includes(email.toLowerCase()) && password === adminPassword) {
      onLogin('admin');
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-[#f3f4f6] flex items-center justify-center p-2 sm:p-4 overflow-hidden">
      {/* Container: max-h-screen ve dikeyde sığacak şekilde ayarlandı */}
      <div className="w-full max-w-2xl bg-white rounded-[3rem] sm:rounded-[4rem] shadow-[0_40px_100px_-20px_rgba(0,0,0,0.3)] overflow-hidden animate-in zoom-in-95 duration-500 max-h-full sm:max-h-[90vh] flex flex-col border border-gray-100">
        
        {/* Header - Daha dar tutuldu (p-8 sm:p-12) */}
        <div className="bg-[#1D2D4C] p-6 sm:p-10 text-center relative overflow-hidden flex-shrink-0">
          ...
          <div className="relative z-10 mb-4 sm:mb-6 flex justify-center">
            <div className="p-3 sm:p-4 bg-white rounded-full shadow-2xl border-4 border-white">
              <Logo className="w-16 h-16 sm:w-28 sm:h-28" />
            </div>
          </div>

          <h1 className="text-white text-3xl sm:text-5xl font-black italic uppercase tracking-tighter relative z-10 leading-none">
            BATMAN <span className="text-[#E30613]">GENÇLERBİRLİĞİ</span>
          </h1>
          <p className="text-slate-400 text-[9px] sm:text-[12px] font-black uppercase tracking-[0.4em] mt-3 italic relative z-10 opacity-70">AKADEMİ YÖNETİMİ</p>
        </div>

        {/* Form Alanı - Esnek ama tek ekrana odaklı */}
          ...
          {view === 'login' ? (
            <form onSubmit={handleLogin} className="space-y-6 sm:space-y-8 h-full flex flex-col justify-center">
              <div className="space-y-1 text-center">
                ...
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Resmi akademi erişimi.</p>
              </div>
              ...
              <div className="pt-6 border-t border-gray-100 flex flex-col items-center gap-4">
                 <p className="text-[11px] font-black text-slate-900 uppercase tracking-widest text-center italic">
                   GELECEĞİN YILDIZI OLMAYA HAZIR MISIN?
                 </p>
            ...
            <form onSubmit={handleRegister} className="space-y-6 sm:space-y-8">
              <div className="space-y-1">
                <h2 className="text-3xl font-black uppercase italic tracking-tighter text-slate-900">YENİ <span className="text-[#E30613]">KAYIT</span></h2>
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Akademiye ilk adımı atın.</p>
              </div>

              <div className="space-y-4 max-h-[300px] sm:max-h-none overflow-y-auto no-scrollbar">
              ...
              <div className="space-y-3">
                <button type="submit" className="w-full py-5 bg-[#E30613] text-white rounded-[2rem] font-black text-sm uppercase tracking-widest shadow-2xl flex items-center justify-center gap-3 hover:bg-[#1D2D4C] transition-all active:scale-[0.97]">
                  BAŞVURUYU TAMAMLA <CheckCircle2 size={20} />
                </button>
                <button type="button" onClick={() => setView('login')} className="w-full text-[10px] font-black text-gray-400 uppercase tracking-[0.3em] text-center">GERİ DÖN</button>
              </div>
            </form>
          )}
        </div>
